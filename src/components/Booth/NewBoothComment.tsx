...
const NewBoothComment: React.FC<NewBoothCommentProps> = ({nickname, onClick, onCommentSubmit}) => {
  const [content, setContent] = useState<string>(""); // 댓글 내용 상태
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false); // 댓글 작성 중인지 상태
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setIsSubmitting(false); // 댓글 작성 완료
    }
  };
  ...
  return (
    <div className="mt-20 w-full flex flex-col items-center space-y-2">
      ...
      {/* 댓글 작성란 */}
      <div onClick={onClick} className="relative w-full max-w-[500px] h-auto bg-white rounded-[15px] pl-4 pr-16 py-2 flex flex-col justify-center items-start gap-1">
